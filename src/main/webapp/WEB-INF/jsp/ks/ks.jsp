...
<div class="ks_left">
	<div class="ks_left_top">
        ...
        <div class="red" id="remainTime">剩余时间<span id="tMinute">${testDTO.examDTO.time}</span>分钟 &nbsp;<span id="tSecond">0</span>秒</div>
    </div>
    ...
    <div class="ks_left_down">
    	<div class="xuanti">
            <c:forEach var="pageNo" begin="1" end="${testDTO.pageBean.rowsCount}" step="1">
                ...
                <c:if test="${pageNo != testDTO.pageBean.curPage}">
                    <a href="/test/user/${testDTO.userDTO.id}/pageNo/${pageNo}" class="a_white" id="q${testDTO.ids[pageNo-1]}">${pageNo}</a>
                </c:if>
            </c:forEach>
...
<div class="ks_right">
    ...
    <c:forEach var="answermap" items="${answerMap}">
...
<script type="text/javascript">
    $(document).ready(
            function () {
                if(${testDTO.pageBean.curPage==1}){
                    $("#pre").attr("disabled",true);
                }
                //上一题下一题提交
             //   $("#pre ,#next").click(function(){
             //       if($(this).attr("id")==="pre"){
            //            $("form").attr("action","/test/user/${testDTO.userDTO.id}/pageNo/${testDTO.pageBean.curPage-1}");
            //        }
            //        if($(this).attr("id")==="next"){
            //            if(${testDTO.pageBean.curPage == testDTO.pageBean.rowsCount})
            //           $("form").attr("action","/test/user/${testDTO.userDTO.id}/pageNo/${testDTO.pageBean.curPage}");
            //            else {
            //                $("form").attr("action","/test/user/${testDTO.userDTO.id}/pageNo/${testDTO.pageBean.curPage+1}");
            //            }
            //        }
            //    });
                   $(".a_green , .a_white ,#pre ,#next").click(function(){
                       var url ="";
                       var questionId="";
                       var questionAnswer="";
                       var data;
                       ...
                       if($(this).attr("class")==="a_green"||$(this).attr("class")==="a_white"){
                            url = $(this).attr("href");
                       }
                       if($(this).attr("id")==="pre"){
...
//显示问题页面
                        $.post(url,data,function(testDTO){
                            $("#tMinute").html(testDTO.tMinute);
                            $("#tSecond").html(testDTO.tSecond);
