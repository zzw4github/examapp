    	...
    	<div class="xuanti">
            <c:forEach var="pageNo" begin="1" end="${fn:length(resultDTO.questionDTOs)}" step="1">
                <c:if test="${resultDTO.questionDTOs[pageNo-1].tof == 0}">
                   <a href="/test/user/${testDTO.userDTO.id}/question/${testDTO.ids[pageNo-1]}" class="a_red" id="q${testDTO.ids[pageNo-1]}">${pageNo}</a>
                </c:if>
                <c:if test="${resultDTO.questionDTOs[pageNo-1].tof == 1}">
                    <a href="/test/user/${testDTO.userDTO.id}/question/${testDTO.ids[pageNo-1]}" class="a_green" id="q${testDTO.ids[pageNo-1]}">${pageNo}</a>
                </c:if>
            </c:forEach>
    	</div> 
...
<div class="ks_right">
    ...
    <a href="/test/user/${testDTO.userId}/question/${resultDTO.questionDTOs[1].id}" style="display: none" id="nextUrl"></a>
</div>
...
//                      }
                      ...
                      $("#nextUrl").attr("hfre","/test/user/"+result.userId+"/question/"+result.nextId);
                  }
          );
      });
    });
